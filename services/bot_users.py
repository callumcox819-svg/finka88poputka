...
from services.domain_list import ensure_finland_default_domains
from services.user_settings import get_setting, set_setting


async def _seed_finland_profile_defaults(user_id: int) -> None:
    await ensure_finland_default_domains(user_id)
...
async def get_or_create_bot_user(telegram_id: int) -> dict:
    ...
    async with db_connect() as db:
        ...
        if row:
            await _seed_finland_profile_defaults(tid)
            return row.as_dict()
        await db.execute(
            f"""
